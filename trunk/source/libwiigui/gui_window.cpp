...
void GuiWindow::SetBlackbox(bool s, int x, int y,int w,int h,GXColor c)
{
	bbx= x;
	bby= y;
	bbw= w;
...
void GuiWindow::Draw()
{
	...
	if (blackbox)
		Menu_DrawRectangle(bbx,bby,bbw,bbh,bbc,1);

	if(parentElement && state == STATE_DISABLED)
		Menu_DrawRectangle(0,0,screenwidth,screenheight,(GXColor){69,1,94, 0x70},1);
